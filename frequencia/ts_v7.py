...
def comparar(df_freq, df_rh):
    ...
    sec = (
        df_freq.groupby(["funcional","nome","data","ocorrencia"])
        .size()
        .reset_index(name="qtd_secretaria")
    )

    rh = (
        df_rh.groupby(["funcional","nome","data","ocorrencia"])
        .size()
        .reset_index(name="qtd_rh")
    )

    merged = pd.merge(
        rh,
        sec,
        on=["funcional","nome","data","ocorrencia"],
        how="outer"
    )
    ...
    for _, row in merged.iterrows():
        ...
        if row["qtd_rh"] > 0 and row["qtd_secretaria"] > 0:
            status = "OK"
        else:
            status = "DIVERGENTE"
