    ...
    for paragraph in document.paragraphs:
        ...
        for key, value in replacements.items():
            ...
            output_filename = os.path.join(
                output_dir,
                f'{funcionario_raw}.docx'
            )

            document.save(output_filename)    print(f'Documento gerado: {output_filename}')
...
for index, row in df.iterrows():
    ...
    # Lê a coluna condição (seguro contra NaN)
    condicao = ''
    ...
    if condicao == 'não enviar':
        print(f"Pulando {funcionario} (condição: não enviar)")
        continue

    elif condicao == 'desligado':
        template_escolhido = 'template_desligado.docx'

    else:
        # vazio ou qualquer outra coisa
        template_escolhido = 'template_ligado.docx'
